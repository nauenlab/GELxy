...
while True:
    for angle in range(0, int(2.0*pi/angle_delta)):
        xx = int(radius*math.cos(angle * angle_delta))
        yy = int(radius * math.sin(angle * angle_delta))
        
        while xx!=x or yy!=y:
            if xx>x:
                #Move the xmotor clockwise
                x += 1 
            elif xx < x:
                #Move the xmotor counterclockwise
                x -= 1 
                
            if yy > y:
                   #Move the ymotor clockwise 
                    
            elif yy < y:
                #Move the ymotor counterclockwise 
                      y -= 1
                        
                    #simulate the delay 
                    time.sleep(DELAY / 1000.0)
                    
                #perform action related to current angle, if needed 
                
                #Add break statement here to exit the loop after one cycle 
                

for (float angle = 10.0; angle < 2.0*pi; angle += angle_delta)
...
# Set step size and velocity for jog for stage/motor (decimal max velocity, decimal acceleration)
deviceY.SetJogStepSize(Decimal(xx))
deviceY.SetJogVelocityParams(Decimal(0.5), Decimal(1.0))
deviceX.SetJogStepSize(Decimal(xx))
deviceX.SetJogVelocityParams(Decimal(0.5), Decimal(1.0))
